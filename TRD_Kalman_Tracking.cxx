...
vector<vector<Double_t>> TRD_Kalman_Trackfinder::get_Kalman_helix_params() { return mHelices; }
vector<Double_t> TRD_Kalman_Trackfinder::get_Kalman_chi_2() { return mChi_2s; }
...
Bool_t TRD_Kalman_Trackfinder::fitting(Ali_TRD_ST_Tracklets* a, Ali_TRD_ST_Tracklets* b) {
    ...
    /*}
     */
    if (abs(a_angle.Angle(b_angle) * TMath::RadToDeg()) > 15) return 0;
    if (abs(dir_a.Angle(dir_b) * TMath::RadToDeg()) > 15) return 0;
...
void TRD_Kalman_Trackfinder::Kalman(vector<Ali_TRD_ST_Tracklets*> seed) {
    {  // init
        ...
        mCov[3][3] =
            TMath::Power(TMath::Tan(dsin_theta * TMath::Pi() / 180.0), 2);  // 20.0
        ...
        if (mMeasurements[i_layer] != 0) {
            ...
            for (const auto i_det : Dets) {
                ...
                for (Int_t i_tracklet = 0; i_tracklet < (Int_t)mBins[i_det].size(); i_tracklet++) {
                    ...
                    if (mShow) {
                        cout << "meas" << measurement << ", " << mBins[i_det][i_tracklet]->get_TV3_offset()[0] << ", "
                             << mBins[i_det][i_tracklet]->get_TRD_index() << endl;
                        cout << "chi:" << chi_2 << endl;
                    }
                    ...
                    // cout<<"dist to rad new:"<<mTRD_layer_radii_all[mCurrent_Det]-mBins[i_det][i_tracklet]->get_TV3_offset()[0]<<" dtor
                    // old:"<<mTRD_layer_radii[mCurrent_Det%6][1]-mBins[i_det][i_tracklet]->get_TV3_offset()[0]<<endl;
...
vector<vector<Ali_TRD_ST_Tracklets*>> TRD_Kalman_Trackfinder::Kalman_Trackfit(vector<vector<Ali_TRD_ST_Tracklets*>> tracks,
                                                                              Int_t prim_vertex) {
    mShow = 0;
    mSearch_tracklets = 0;
    mHelices.clear();
