...
static void sum_distance_circ_point_2D(Int_t&, Double_t*, Double_t& sum, Double_t* par, Int_t) {
    ...
    for (Int_t i_layer = 0; i_layer < 6; ++i_layer) {
        ...
        // \n",i_layer,vec_Dt_digit_pos_cluster[6][i_layer][0],vec_Dt_digit_pos_cluster[6][i_layer][1],sum); printf("i_layer: %d, d: %4.8f
        // \n",i_layer,d);
    }
}
#endif
...
Ali_TRD_ST_Analyze::Ali_TRD_ST_Analyze(TString out_dir, TString out_file_name, Int_t graphics) {
    ...
    // NT_secondary_vertices = new TNtuple("NT_secondary_vertices","NT_secondary_vertices
    // Ntuple","x:y:z:ntracks:pT_AB:qpT_A:qpT_B:AP_pT:AP_alpha:dcaTPC:pathTPC:InvM:Eta:Phi:GlobEv:dotprod:TPCdEdx_A:dca_TPC_A:p_TPC_A:TPCdEdx_B:dca_TPC_B:p_TPC_B:InvMK0s:dcaAB:InvML:InvMaL");
    // NT_secondary_vertices ->SetAutoSave( 5000000 );

    // NT_secondary_vertex_cluster = new TNtuple("NT_secondary_vertex_cluster","NT_secondary_vertex_cluster
    // Ntuple","x:y:z:nvertices:dcaTPC:tof:trklength:dEdx:dcaprim:pT:mom:layerbitmap"); NT_secondary_vertex_cluster ->SetAutoSave( 5000000
    // );
...
#endif
    if (1) h1D_invariant_mass_S = new TH1D("h1D_invariant_mass_S", "h1D_invariant_mass_S", 500, 0, 5);
...
#endif
            for (Int_t i_vertex = 0; i_vertex < 8; i_vertex++) {
                Double_t arr_pos_glb[3] = {vec_TH1D_TRD_geometry[0][i_vertex]->GetBinContent(TRD_detector),
                                           vec_TH1D_TRD_geometry[1][i_vertex]->GetBinContent(TRD_detector),
                                           vec_TH1D_TRD_geometry[2][i_vertex]->GetBinContent(TRD_detector)};
    ...
    for (Int_t i_det = 0; i_det < 540; i_det++) {
        ...
        // printf("i_det: %d, i_det from tg: %4.3f, n_cols: %4.3f, n_rows: %4.3f, col start: %4.3f, col end: %4.3frow start: %4.3f, row end:
        // %4.3f \n", i_det,geom
        // ->GetY()[0],vec_trd_TRD_pp_geom[i_det][0],vec_trd_TRD_pp_geom[i_det][1],vec_trd_TRD_pp_geom[i_det][2],vec_trd_TRD_pp_geom[i_det][3],vec_trd_TRD_pp_geom[i_det][4],vec_trd_TRD_pp_geom[i_det][5]);
    }
    ...
    for (Int_t i_det = 0; i_det < 540; i_det++) {
        ...
        if (n_row == 0 || n_col == 0) {
            vec_tp2d_gain_vs_xz[i_det] = new TProfile2D(Form("vec_tp2d_gain_vs_xz%d", i_det), Form("vec_tp2d_gain_vs_xz%d", i_det), 144,
                                                        -60.0, 60.0, 16, -60.0, 60.0);
        }
            ...
            vec_tp2d_gain_vs_xz[i_det] = new TProfile2D(Form("vec_tp2d_gain_vs_xz%d", i_det), Form("vec_tp2d_gain_vs_xz%d", i_det), n_col,
                                                        -half_x, half_x, n_row, -half_z, half_z);
        }
    }
}
//----------------------------------------------------------------------------------------
...
void Ali_TRD_ST_Analyze::fHelixAtoPointdca(TVector3 space_vec, Ali_Helix* helixA, Float_t& pathA, Float_t& dcaAB) {
    ...
    while (fabs(scale_length) > 0.01 && loopcounter < 100)  // stops when the length is too small
    {
        ...
        if (distarray[0] > distarray[1]) {
            if (loopcounter != 0) {
                if (flip == 1.0)
                    scale = 0.4;  // if minimization direction changes -> go back, but only the way * 0.4
                else
                    scale = 0.7;  // go on in this direction but only by the way * 0.7
            }
...
std::pair<Float_t, Float_t> Ali_TRD_ST_Analyze::fHelixAtoPointdca_xy_z(TVector3 space_vec, Ali_Helix* helixA, Float_t& pathA,
                                                                       Float_t& dcaAB_xy, Float_t& dcaAB_z) {
    // V1.1
    Float_t pA[2] = {100.0, -100.0};  // the two start values for pathB, 0.0 is the origin of the helix at the first measured point
    Float_t distarray[2];
...
//-----------------------------------------------------------------------------------------
Int_t Ali_TRD_ST_Analyze::fCircle_Interception(Double_t x1, Double_t y1, Double_t r1, Double_t x2, Double_t y2, Double_t r2, Double_t& x1_c,
...
pair<Double_t, Double_t> Ali_TRD_ST_Analyze::fpathLength(Double_t r, Ali_Helix* helixA) const {
    ...
    Double_t t35 = t14 * t3;
    Double_t t38 = 8.0 * t4 * t6 - 4.0 * t1 * t2 * t8 - 4.0 * t11 * curvature * t6 + 4.0 * t15 * t6 + t17 * t3 + t19 * t3 + 2.0 * t21 * t8 +
                   4.0 * t8 * t23 - 4.0 * t8 * x0 * curvature * t5 - 4.0 * t11 * t23 - 4.0 * t11 * y0 * curvature * t2 + 4.0 * t11 -
                   4.0 * t14 + t32 * t3 + 4.0 * t15 * t4 - 2.0 * t35 * t11 - 2.0 * t35 * t8;
    // cout<<t38<<endl;
    Double_t t40 = (-t3 * t38);
    ...
    if (value.first > value.second) swap(value.first, value.second);
    return (value);
}
//----------------------------------------------------------------------------------------
...
TVector3 Ali_TRD_ST_Analyze::fDCA_Helix_Estimate(Ali_Helix* helixA, Ali_Helix* helixB, Float_t& pathA, Float_t& pathB, Float_t& dcaAB) {
    ...
    // printf("2D circle cross points, circle: {x1: %4.3f, y1: %4.3f, r1: %4.3f} {x2: %4.3f, y2: %4.3f, r2: %4.3f} (Sven): {%4.3f, %4.3f},
    ...
    //     << "}, xyr(2) = {"  << x2 << ", " << y2 << ", " << r2 << "}, p1 = {" << x1_c << ", " << y1_c << "}, p2 = {" << x2_c << ", " <<
    //     y2_c << "}" << endl;
...
//--------------------------------------------------------------------------------------------
TVector3 Ali_TRD_ST_Analyze::fDCA_Helix_Estimate_xy_z(Ali_Helix* helixA, Ali_Helix* helixB, Float_t& pathA, Float_t& pathB,
                                                      Float_t& dcaAB_xy, Float_t& dcaAB_z) {
    ...
    TVector3 pointA, pointB, pointA1, pointB1, pointA2, pointB2, pointA1_xy, pointA1_z, pointA2_xy, pointA2_z, pointB1_xy, pointB1_z,
        pointB2_xy, pointB2_z;
...
//----------------------------------------------------------------------------------------
// V1.0
void Ali_TRD_ST_Analyze::fHelixABdca(Ali_Helix* helixA, Ali_Helix* helixB, Float_t& pathA, Float_t& pathB, Float_t& dcaAB, Float_t pathA_in,
                                     Float_t pathB_in) {
...
Int_t Ali_TRD_ST_Analyze::Calculate_secondary_vertices(Int_t graphics, Int_t flag_TRD_TPC_tracks, Int_t flag_fill_tree) {
    ...
    for (Int_t i_track_A = 0; i_track_A < ((Int_t)vec_helices.size() - 1); i_track_A++)  // TRD Kalman track A
    {
        ...
        for (Int_t i_track_B = (i_track_A + 1); i_track_B < (Int_t)vec_helices.size(); i_track_B++)  // TRD Kalman track B
        {
            ...
            TVector3 TV3_photon =
                fDCA_Helix_Estimate_xy_z(vec_helices[i_track_A], vec_helices[i_track_B], pathA_est, pathB_est, dcaAB_est_xy, dcaAB_est_z);
            ...
            // if(dcaAB < 5.0)
            {
                ...
                Float_t pathA_photon, pathB_photon, dcaA_photon, dcaB_photon, dcaA_photon_xy, dcaB_photon_xy, dcaA_photon_z, dcaB_photon_z,
                    dcaAB_xy, dcaAB_z;
                fHelixAtoPointdca_xy_z(TV3_photon, vec_helices[i_track_A], pathA_photon, dcaA_photon_xy,
                                       dcaA_photon_z);  // new helix to point dca calculation
                fHelixAtoPointdca_xy_z(TV3_photon, vec_helices[i_track_B], pathB_photon, dcaB_photon_xy,
                                       dcaB_photon_z);  // new helix to point dca calculation
                pathA = pathA_photon;
                pathB = pathB_photon;
                ...
                // if(i_comb == 38)
                {
                    ...
                    if (radius_vertex > min_radius_cut && radius_vertex < max_radius_cut)  // TRD acceptance in R-direction
                    {
                        ...
                        TV3_dirA.SetXYZ(helix_pointAs[0] - helix_pointA[0], helix_pointAs[1] - helix_pointA[1],
                                        helix_pointAs[2] - helix_pointA[2]);
                        TV3_dirB.SetXYZ(helix_pointBs[0] - helix_pointB[0], helix_pointBs[1] - helix_pointB[1],
                                        helix_pointBs[2] - helix_pointB[2]);
                        ...
                        // if(fabs(AP_alpha) < 0.2 && AP_pT > 0.0 && AP_pT < 0.02 && CA*CB < 0.0 && pTA > 0.04 && pTB > 0.04 && pTA < 0.5 &&
                        // pTB < 0.5 && dot_product_dir_vertex > 0.9) // TRD photon conversion printf("AP_value: %4.3f,
                        // dot_product_dir_vertex: %4.3f, CA*CB: %4.3f, pTA: %4.3f, pTB: %4.3f
                        ...
                        if (AP_value < AP_cut_value && dot_product_dir_vertex > 0.7) {
                            ...
                            if (flag_TRD_TPC_tracks == 0)  // TRD track
                            {
                                ...
                                for (Int_t i_track = 0; i_track < NumTracks; i_track++) {
                                    ...
                                    TPC_single_helix->setHelix(TRD_ST_TPC_Track->getHelix_param(0), TRD_ST_TPC_Track->getHelix_param(1),
                                                               TRD_ST_TPC_Track->getHelix_param(2), TRD_ST_TPC_Track->getHelix_param(3),
                                                               TRD_ST_TPC_Track->getHelix_param(4), TRD_ST_TPC_Track->getHelix_param(5));
                                    fHelixAtoPointdca_xy_z(TV3_sec_vertex, TPC_single_helix, pathA_dca, dcaAB_dca_xy,
                                                           dcaAB_dca_z);  // new helix to point dca calculation

                                    Double_t helix_point_TPC_photon[3];
                                    TPC_single_helix->Evaluate(pathA_dca,
                                                               helix_point_TPC_photon);  // 3D-vector of helixB point at path pB[r]
                                    TV3_close_TPC_photon.SetXYZ(helix_point_TPC_photon[0], helix_point_TPC_photon[1],
                                                                helix_point_TPC_photon[2]);
                            ...
                            if (flag_TRD_TPC_tracks == 0)  // TRD track
                            {
                                if (((shared_layer[0] + shared_layer[1] + shared_layer[2]) > 1 &&
                                    ...
                                    ((shared_layer[0] + shared_layer[1] + shared_layer[2]) > 0 &&
                                     (independent_layer_A[5] + independent_layer_A[4] + independent_layer_A[3] + independent_layer_A[2]) >
                                         2 &&
                                     (independent_layer_B[5] + independent_layer_B[4] + independent_layer_B[3] + independent_layer_B[2]) >
                                         2)) {
                                    if (dca_min > 10.0 || (dca_min <= 10.0 && path_min < 0.0))  // no close-by TPC track
                                    {
#if defined(USEEVE)
                                        if (graphics && flag_close_TPC_photon)
                                            TEveP_close_TPC_photon->SetPoint(i_close_TPC_photon, TV3_close_TPC_photon[0],
                                                                             TV3_close_TPC_photon[1], TV3_close_TPC_photon[2]);
#endif
                                        i_close_TPC_photon++;
                                        // printf("%s Number of shared tracklets: %s %d, independent A,B: {%d, %d}: dca_min: %4.3f,
                                        // i_track_min: %d \n",KGRN,KNRM,N_shared_AB, N_independent_AB[0],
                                        // N_independent_AB[1],dca_min,i_track_min); printf("      --> Found vertex for AP in %s event: %lld
                                        // %s at radius: %4.3f, pos: {%4.3f, %4.3f, %4.3f}, AP_pT: %4.3f, AP_alpha: %4.3f, pTA: %4.3f, pTB:
                                        // %4.3f, dot: %4.3f, Inv_mass_AB: %4.3f, Energy_AB: %4.3f, Momentum_AB: %4.3f
                            ...
                            if (flag_TRD_TPC_tracks == 0)  // Kalman
                            {
                                for (Int_t i_trackAB = 0; i_trackAB < 2; i_trackAB++) {
                                    ...
                                    if (i_trackAB == 0) {
                                        ...
                                        TRD_Kalman_track->setKalmanHelix_param(
                                            vec_helices[i_track_A]->getHelix_param(0), vec_helices[i_track_A]->getHelix_param(1),
                                    ...
                                    if (i_trackAB == 1) {
                                        ...
                                        TRD_Kalman_track->setKalmanHelix_param(
                                            vec_helices[i_track_B]->getHelix_param(0), vec_helices[i_track_B]->getHelix_param(1),
                            ...
                            {
#if defined(USEEVE)
                                // if(AP_pT > 0.05 && AP_pT < 0.25 && CA*CB < 0.0 && pTA > 0.2 && pTB > 0.2 && dot_product_dir_vertex >
                                // 0.95) if(fabs(AP_alpha) < 0.2 && AP_pT > 0.0 && AP_pT < 0.02 && CA*CB < 0.0 && pTA > 0.04 && pTB > 0.04
                                // && pTA < 0.5 && pTB < 0.5 && dot_product_dir_vertex > 0.9) // TRD photon conversion if(fabs(AP_alpha) <
                                // 0.2 && AP_pT > 0.0 && AP_pT < 0.02 && CA*CB < 0.0 && pTA > 0.04 && pTB > 0.04 && pTA < 0.2 && pTB < 0.2
                                // && dot_product_dir_vertex > 0.9) // TRD photon conversion if(fabs(AP_alpha) < 0.2 && AP_pT > 0.0 && AP_pT
                                // < 0.02 && CA*CB < 0.0 && pTA > 0.3 && pTB > 0.3 && pTA < 0.5 && pTB < 0.5 && dot_product_dir_vertex >
                                // 0.9) // TPC photon conversion
                                {
                                    // printf("-----> Found vertex for AP in event: %lld at radius: %4.3f, AP_pT: %4.3f, AP_alpha: %4.3f,
                                    // pTA: %4.3f, pTB: %4.3f, dot: %4.3f, Inv_mass_AB: %4.3f, Energy_AB: %4.3f, Momentum_AB: %4.3f
                                    ...
                                    if (graphics) {
                                        ...
                                        // printf("i_comb: %d, A p,pT,pz: {%4.3f, %4.3f, %4.3f}, B p,pT,pz: {%4.3f, %4.3f, %4.3f}, AP pT,
                                        // alpha: {%4.3f, %4.3f}, TV3_dirAB: {%4.3f, %4.3f, %4.3f}
                                        ...
                                        TEveP_second_point_helix->SetPoint(i_vertex_acc, helix_pointAs[0], helix_pointAs[1],
                                                                           helix_pointAs[2]);
                                        TEveP_first_point_helix->SetPoint(i_vertex_acc + 2, helix_pointB[0], helix_pointB[1],
                                                                          helix_pointB[2]);
                                        // PonTEveP_second_point_helix
                                        // ->SetPoint(i_vertex_acc+2,helix_pointBs[0],helix_pointBs[1],helix_pointBs[2]);
                                        ...
                                        TEveLine_mother[i_vertex_acc]->SetNextPoint(
                                            vertex_point[0] - distance_to_prim_vertex * TV3_dirAB[0],
                                            vertex_point[1] - distance_to_prim_vertex * TV3_dirAB[1],
                                            vertex_point[2] - distance_to_prim_vertex * TV3_dirAB[2]);
    ...
    if (flag_TRD_TPC_tracks == 0)  // TRD tracks
    {
        ...
        for (Int_t i_sec_vtx_A = 0; i_sec_vtx_A < (N_sec_vertices - 1); i_sec_vtx_A++) {
            ...
            if (N_close_vertex > 2) {
                ...
                /// if(TV3_avg_sec_vertex.Perp() > 297.0 && flag_close_TPC_track) printf("%s ----> Nuclear interaction vertex at radius:  %s
                /// %4.3f cm, pos: {%4.3f, %4.3f, %4.3f} at event: %lld, i_vertex_nucl_int: %d, N_close_vertex: %d, path_min: %4.3f
                ...
                Float_t nuclev_bitmap =
                    Calc_nuclev_bitmap(vec_idx_kalman_tracks_nuclev);  // Calculates the number of shared/independent tracklets per layer
...
#endif
                if ((Arr_tracklets_layer[5] > 2 && Arr_tracklets_layer[4] > 2 && Arr_tracklets_layer[3] > 2)) {
                    if (TV3_avg_sec_vertex.Perp() > 297.0 && flag_close_TPC_track) {
                        // printf("%s ----> Nuclear interaction vertex at radius:  %s %4.3f cm, pos: {%4.3f, %4.3f, %4.3f} at event: %lld,
                        // i_vertex_nucl_int: %d, N_close_vertex: %d, path_min: %4.3f, trkl: {%d, %d, %d, %d, %d, %d}
                ...
                TRD_Nuclear_interaction->set_avg_sec_vertex((Float_t)TV3_avg_sec_vertex[0], (Float_t)TV3_avg_sec_vertex[1],
                                                            (Float_t)TV3_avg_sec_vertex[2]);
                ...
                if (flag_TRD_TPC_tracks == 0)  // Kalman
                {
                    for (Int_t i_track = 0; i_track < (Int_t)vec_idx_kalman_tracks_nuclev.size(); i_track++) {
                        ...
                        TRD_Kalman_track->setKalmanHelix_param(vec_helices[vec_idx_kalman_tracks_nuclev[i_track]]->getHelix_param(0),
                                                               vec_helices[vec_idx_kalman_tracks_nuclev[i_track]]->getHelix_param(1),
                                                               vec_helices[vec_idx_kalman_tracks_nuclev[i_track]]->getHelix_param(2),
                                                               vec_helices[vec_idx_kalman_tracks_nuclev[i_track]]->getHelix_param(3),
                                                               vec_helices[vec_idx_kalman_tracks_nuclev[i_track]]->getHelix_param(4),
                                                               vec_helices[vec_idx_kalman_tracks_nuclev[i_track]]->getHelix_param(5));
                    }
                }
                if (flag_TRD_TPC_tracks == 1)  // TPC
                {
                    for (Int_t i_track = 0; i_track < (Int_t)vec_idx_kalman_tracks_nuclev.size(); i_track++) {
                        ...
                        TPC_track->setHelix(vec_helices[vec_idx_kalman_tracks_nuclev[i_track]]->getHelix_param(0),
                                            vec_helices[vec_idx_kalman_tracks_nuclev[i_track]]->getHelix_param(1),
                                            vec_helices[vec_idx_kalman_tracks_nuclev[i_track]]->getHelix_param(2),
                                            vec_helices[vec_idx_kalman_tracks_nuclev[i_track]]->getHelix_param(3),
                                            vec_helices[vec_idx_kalman_tracks_nuclev[i_track]]->getHelix_param(4),
...
#if defined(USEEVE)
                if (graphics) {
                    TEveP_nucl_int_vertices->SetPoint(i_vertex_nucl_int, TV3_avg_sec_vertex[0], TV3_avg_sec_vertex[1],
                                                      TV3_avg_sec_vertex[2]);
...
void Ali_TRD_ST_Analyze::set_Kalman_helix_params(vector<vector<Double_t>> mHelices_kalman_in) {
    ...
    for (Int_t i_track = 0; i_track < (Int_t)mHelices_kalman.size(); i_track++) {
        ...
        vec_helices_TRD[i_track]->setHelix(mHelices_kalman[i_track][0], mHelices_kalman[i_track][1], mHelices_kalman[i_track][2],
                                           mHelices_kalman[i_track][3], mHelices_kalman[i_track][4], mHelices_kalman[i_track][5]);
...
Int_t Ali_TRD_ST_Analyze::set_TPC_helix_params(Long64_t i_event) {
    ...
    for (Int_t i_track = 0; i_track < NumTracks; i_track++) {
        ...
        vec_helices_TPC[(Int_t)vec_helices_TPC.size() - 1]->setHelix(
            TRD_ST_TPC_Track->getHelix_param(0), TRD_ST_TPC_Track->getHelix_param(1), TRD_ST_TPC_Track->getHelix_param(2),
            TRD_ST_TPC_Track->getHelix_param(3), TRD_ST_TPC_Track->getHelix_param(4), TRD_ST_TPC_Track->getHelix_param(5));
    ...
    for (Int_t i_TPC_track = 0; i_TPC_track < N_TPC_tracks; i_TPC_track++) {
        ...
        if (N_good_TPC_TRD_tracklets > 3)  // Use only TPC tracks with a certain amount of TRD matched tracklets
        {
            ...
            for (Int_t index_kalman_track = 0; index_kalman_track < (Int_t)index_kalman_track_matched.size(); index_kalman_track++) {
                ...
                Double_t matches_percent = 100.0 * (Double_t)vec_N_layers_matched[index_kalman_track_matched[index_kalman_track]] /
                                           (Double_t)N_good_TPC_TRD_tracklets;
            ...
            {
                for (Int_t index_kalman_track = index_kalman_track_best; index_kalman_track <= index_kalman_track_best;
                     index_kalman_track++) {
                    // Fraction of matched Kalman to TPC track TRD tracklets
                    ...
                    if (matches_percent >= 50.0) {
                        ...
                        if (pT_track != 0.0) {
                            ...
                            // printf("%s i_TPC_track: %d %s matched to i_kalm_track: %d, %4.2f%% tracklets matched, delta_pT: %4.3f,
                            // pT(TPC): %4.3f, pT(Kalman): %4.3f, N_TPC_tracklets: %d
...
//----------------------------------------------------------------------------------------
void Ali_TRD_ST_Analyze::Match_kalman_tracks_to_TPC_tracks(Int_t graphics, Int_t draw_matched_TPC_track, Int_t draw_matched_TRD_track,
                                                           Int_t color) {
    // printf("size: %d \n",(Int_t)vec_kalman_TRD_trackets.size());
    //  Loop over kalman matched TRD tracklets
    for (Int_t i_kalm_track = 0; i_kalm_track < (Int_t)vec_kalman_TRD_trackets.size(); i_kalm_track++) {
        ...
        if (N_layers_matched_all >= 2) {
            ...
            for (Int_t index_TPC_track = 0; index_TPC_track < (Int_t)index_TPC_track_matched.size(); index_TPC_track++) {
                Double_t matches_percent =
                    100.0 * (Double_t)vec_N_layers_matched[index_TPC_track_matched[index_TPC_track]] / (Double_t)N_good_kalman_tracklets;
                ...
                if (matches_percent >= 74.0 && vec_N_layers_matched[index_TPC_track_matched[index_TPC_track]] >= 4) {
                    ...
                    vec_TH2D_pT_TPC_vs_Kalman[N_good_kalman_tracklets]->Fill(-TMath::Sign(1, curv_kalman) * pT_kalman,
                                                                             TMath::Sign(1, dca) * pT_track);
                    if ((-TMath::Sign(1, curv_kalman) * pT_kalman) != 0.0 && pT_track != 0.0)
                        vec_TH2D_one_over_pT_TPC_vs_Kalman[N_good_kalman_tracklets]->Fill(1.0 / (-TMath::Sign(1, curv_kalman) * pT_kalman),
                                                                                          1.0 / (TMath::Sign(1, dca) * pT_track));
...
void Ali_TRD_ST_Analyze::Draw_Kalman_Helix_Tracks(Int_t n_track, Int_t color, Double_t low_R, Double_t high_R) {
    ...
    for (Int_t i_track = i_track_start; i_track < i_track_stop; i_track++) {
        ...
        for (Int_t i_layer = 0; i_layer < 6; i_layer++) {
            ...
            fHelixAtoPointdca(TV3_tracklet_off_vector, vec_helices_TRD[i_track], pathA_dca,
                              dcaAB_dca);  // new helix to point dca calculation
        ...
        if (in) {
            ...
            while (in) {
                ...
                if (str[0] != 0) {
                    ...
                    cout << "File added to data chain: " << addfile.Data() << " with " << (file_entries - entries_save) << " entries"
                         << endl;
...
void Ali_TRD_ST_Analyze::Scan_MC_Event(Int_t graphics, Int_t bool_make_invariant_mass_hist_up_to_layer) {
    ...
    std::map<Int_t, std::string> PDG_codes = {{11, "e-"},
                                              {-11, "e+"},
                                              {13, "mu-"},
                                              {-13, "mu+"},
                                              {22, "gamma"},
    ...
    std::map<Int_t, Double_t> PDG_masses = {{11, 0.510998950}, {-11, 0.510998950}, {111, 0.1349768}, {-111, 0.1349768}, {211, 0.13957},
                                            {-211, 0.13957},   {2112, 0.9395},     {-2112, 0.9395},  {2212, 0.938272},  {-2212, 0.938272}};
    ...
    for (Int_t i_track_A = 0; i_track_A < NumTracks; i_track_A++) {
        ...
        printf("MC_track %d with PDG number %d, index %d, partice: %s, mother id %d, vertex: (%4.2f,%4.2f,%4.2f), p: %4.3f\n", i_track_A,
               MC_PDG_code, MC_index_particle, PDG_codes[MC_PDG_code].c_str(), MC_index_mother, TV3_MC_particle_vertex.X(),
               TV3_MC_particle_vertex.Y(), TV3_MC_particle_vertex.Z(), TLV_MC_particle.P());
            ...
            if (!(TMath::Abs(Mother_PDG_code) == 211 || TMath::Abs(Mother_PDG_code) == 2212 || TMath::Abs(Mother_PDG_code) == 321)) {
                for (auto const& indVector : vec_indices_per_generation) {
                    ...
                    if (it != std::end(indVector)) {
                        ...
                        if (pos_index > bool_make_invariant_mass_hist_up_to_layer) break;
                        ...
                        if (possible_reconstructed_mass) {
                            if (map_matched_mc_to_tpc.find(MC_index_particle) != map_matched_mc_to_tpc.end()) {
                                ...
                                TLV_MC_particle_rec.SetPtEtaPhiM(TLV_MC_particle_rec.Pt(), TLV_MC_particle_rec.Eta(),
                                                                 TLV_MC_particle_rec.Phi(), PDG_masses[MC_PDG_code]);

                                vec_inv_masses_per_generation_reconstructed[pos_index].push_back(TLV_MC_particle_rec);

                            } else if (TMath::Abs(MC_PDG_code) == 211 || TMath::Abs(MC_PDG_code) == 2212 ||
                                       TMath::Abs(MC_PDG_code) == 321 || pos_index == bool_make_invariant_mass_hist_up_to_layer) {
                            ...
                            if (MC_PDG_code_gammas[i_gamma] == 22) {
                                ...
                                if (MC_N_daughters_gamma == 2)  // electron and positron
                                {
                                    for (Int_t i_daughter_gamma = 0; i_daughter_gamma < MC_N_daughters_gamma; i_daughter_gamma++) {
                                        arr_index_daughters_gamma[i_daughter_gamma] =
                                            TRD_MC_Track_gammas[i_gamma]->get_arr_index_daughters(i_daughter_gamma);
                                    }
                                    ...
                                    for (Int_t i_electron = 0; i_electron < 2; i_electron++) {
                                        for (Int_t i_track_scanB = 0; i_track_scanB < NumTracks; i_track_scanB++) {
                                            ...
                                            Int_t MC_index_particle_scanB =
                                                TRD_MC_Track_electrons[i_electron + 2 * i_gamma]->get_index_particle();
                                            if (MC_index_particle_scanB == arr_index_daughters_gamma[i_electron]) {
                                                MC_PDG_code_electrons[i_electron] =
                                                    TRD_MC_Track_electrons[i_electron + 2 * i_gamma]->get_PDGcode();
                                                if (MC_PDG_code_electrons[i_electron] == 11 ||
                                                    MC_PDG_code_electrons[i_electron] == -11)  // electron or positron
                                                {
                                                    TLV_MC_particle = TRD_MC_Track_electrons[i_electron + 2 * i_gamma]->get_TLV_particle();
                                                    TV3_MC_particle_vertex =
                                                        TRD_MC_Track_electrons[i_electron + 2 * i_gamma]->get_TV3_particle_vertex();
                                                    Double_t radius = TMath::Sqrt(TMath::Power(TV3_MC_particle_vertex[0], 2) +
                                                                                  TMath::Power(TV3_MC_particle_vertex[1], 2));
                                                    // printf("      --> Electron/positron found, i_electron: %d, PDG code: %d, P: %4.3f
                                                    // GeV/c, eta: %4.3f, radius: %4.3f
                                                    ...
                                                    if (TLV_MC_particle.P() > 0.002) {
                                                        ...
                                                        vector<Double_t> fhelix = Get_Helix_params_from_kine(
                                                            TLV_MC_particle, TV3_MC_particle_vertex, -1.0 * charge);
                                                        TRD_ST_TPC_Track_MC->setHelix(fhelix[0], fhelix[1], fhelix[2], fhelix[3], fhelix[4],
                                                                                      fhelix[5]);
#if defined(USEEVE)
                                                        if (graphics) {
    ...
    if (bool_make_invariant_mass_hist_up_to_layer) {
        for (Int_t i_gen = bool_make_invariant_mass_hist_up_to_layer; i_gen >= 1; i_gen--) {
            for (Int_t i_particle = 0; i_particle < (Int_t)vec_indices_per_generation[i_gen].size(); i_particle++) {
                ...
                TRD_MC_Track_pi0 =
                    TRD_ST_Event->getMCparticle(index_particle_to_track_number[vec_indices_per_generation[i_gen][i_particle]]);
                Int_t MC_index_mother = TRD_MC_Track_pi0->get_index_mother();

                auto it = std::find(std::begin(vec_indices_per_generation[i_gen - 1]), std::end(vec_indices_per_generation[i_gen - 1]),
                                    MC_index_mother);
                ...
                printf("i_gen %d, ind mother %d, own ind %d, pos %d, i_part %d\n", i_gen, MC_index_mother,
                       TRD_MC_Track_pi0->get_index_particle(), pos_l_vec, i_particle);
                vec_inv_masses_per_generation[i_gen - 1][pos_l_vec] += vec_inv_masses_per_generation[i_gen][i_particle];

                if (possible_reconstructed_mass) {
                    vec_inv_masses_per_generation_reconstructed[i_gen - 1][pos_l_vec] +=
                        vec_inv_masses_per_generation_reconstructed[i_gen][i_particle];
                }
                ...
                if (TMath::Abs(TRD_MC_Track_pi0->get_PDGcode()) == 3122) {
                    ...
                    h2d_invariant_mass_vs_R_Lambda->Fill(vec_inv_masses_per_generation[i_gen][i_particle].M(),
                                                         TV3_MC_particle_vertex.Mag());
                    if (possible_reconstructed_mass)
...
void Ali_TRD_ST_Analyze::Draw_Inv_Mass_histogram() {
    ...
    TCanvas* can_invariant_mass_S_reconstructed =
        new TCanvas("can_invariant_mass_S_reconstructed", "can_invariant_mass_S_reconstructed", 10, 10, 500, 500);
    can_invariant_mass_S_reconstructed->cd();
    h1D_invariant_mass_S_reconstructed->Draw("HIST");

    TCanvas* can_invariant_mass_L_reconstructed =
        new TCanvas("can_invariant_mass_L_reconstructed", "can_invariant_mass_L_reconstructed", 10, 10, 500, 500);
    can_invariant_mass_L_reconstructed->cd();
    h1D_invariant_mass_L_reconstructed->Draw("HIST");

    TCanvas* can_invariant_mass_K_reconstructed =
        new TCanvas("can_invariant_mass_K_reconstructed", "can_invariant_mass_K_reconstructed", 10, 10, 500, 500);
    ...
    TCanvas* can_h2d_invariant_mass_vs_R_Lambda =
        new TCanvas("can_h2d_invariant_mass_vs_R_Lambda", "can_h2d_invariant_mass_vs_R_Lambda", 10, 10, 500, 500);
...
void Ali_TRD_ST_Analyze::Match_TPC_to_MC_Data() {
    ...
    std::map<Int_t, Double_t> PDG_codes_charge = {
        {11, -1.0},    {-11, 1.0},   {13, -1.0},   {-13, 1.0}, {22, 0},      {111, 0},    {113, 0},  {130, 0},  {211, 1.0}, {-211, -1.0},
        {221, 0},      {223, -1.0},  {310, 0},     {321, 1.0}, {-321, -1.0}, {331, 0},    {-331, 0}, {2112, 0}, {-2112, 0}, {2212, 1.0},
        {-2212, -1.0}, {3112, -1.0}, {-3112, 1.0}, {-3122, 0}, {3122, 0},    {3222, 1.0}, {3212, 0}, {-3212, 0}};
    ...
    for (Int_t i_track_tpc = 0; i_track_tpc < NumTPCTracks; i_track_tpc++) {
        ...
        for (Int_t i_track_mc = 0; i_track_mc < NumMCTracks; i_track_mc++) {
            ...
            Float_t Chi_2 = 0;
            for (int i = 0; i < 6; i++) {
                Chi_2 += 0.2 * TMath::Power(Helixparams_diff[i], 2) * Helixparams_cuts[i];
            }
            /*
            if(i_track_tpc==8 && i_track_mc==45 && 1)
            {
               for (int i = 0;i<6 ;i++){ printf("helix_diff: %f, %f,
...
//----------------------------------------------------------------------------------------
void Ali_TRD_ST_Analyze::Draw_MC_track_w_vertices(Int_t i_track_print_nbr, Int_t color, Double_t line_width, Double_t max_path,
                                                  Int_t i_track) {
    ...
    std::map<Int_t, std::string> PDG_codes_names = {{11, "e-"},
                                                    {-11, "e+"},
                                                    {13, "mu-"},
                                                    {-13, "mu+"},
                                                    {22, "gamma"},
    ...
    if (MC_N_daughters > 0 && !(TMath::Abs(MC_PDG_code) == 211 || TMath::Abs(MC_PDG_code) == 2212 || TMath::Abs(MC_PDG_code) == 321)) {
        ...
        TV3_MC_particle_vertex_end =
            TRD_ST_Event->getMCparticle(index_particle_to_track_number[last_daughter_index])->get_TV3_particle_vertex();
    ...
    if (fabs(charge) < 1e-8) {  // neutral particle ... project a straight line
        ...
        Double_t velo_mc[3] = {TLV_MC_particle.Px() / TLV_MC_particle.P(), TLV_MC_particle.Py() / TLV_MC_particle.P(),
                               TLV_MC_particle.Pz() / TLV_MC_particle.P()};
        track_pos[0] = TV3_MC_particle_vertex.X();
        track_pos[1] = TV3_MC_particle_vertex.Y();
        track_pos[2] = TV3_MC_particle_vertex.Z();
        ...
        do {
            ...
            radius_helix = TMath::Sqrt(TMath::Power(track_pos[0], 2) + TMath::Power(track_pos[1], 2));
            dist_to_endvertex = TMath::Sqrt(TMath::Power(TV3_MC_particle_vertex_end.X() - track_pos[0], 2) +
                                            TMath::Power(TV3_MC_particle_vertex_end.Y() - track_pos[1], 2) +
                                            TMath::Power(TV3_MC_particle_vertex_end.Z() - track_pos[2], 2));
            track_path += 1;
...
pair<std::vector<pair<std::vector<Float_t>, Int_t>>, std::vector<pair<std::vector<Float_t>, Int_t>>>
    Ali_TRD_ST_Analyze::NIInteractionCandidateswithHelix(Long64_t i_event) {
    ...
    std::map<Int_t, std::string> dict_mother = {{1000010020, "Deuteron"},       {1000010030, "Triton"},       {1000020030, "He3"},
                                                {1000020040, "Alpha"},          {1000020050, "He5"},          {1000030050, "Li5"},
                                                {-1000010020, "Anti-Deuteron"}, {-1000010030, "Anti-Triton"}, {-1000020030, "Anti-He3"},
                                                {-1000020040, "Anti-alpha"},    {1000020050, "He5"}};

    // map of outgoings in nuclear interactions
    std::map<Int_t, std::string> dict_low = {{2212, "proton"},  {-2212, "anti-proton"},  {311, "K0"},  {321, "K+"},  {-321, "K-"},
                                             {2112, "neutron"}, {-2112, "anti-neutron"}, {111, "pi0"}, {211, "pi+"}, {-211, "pi-"}};
    std::map<Int_t, std::string>::iterator it;

    // charges
    std::map<Int_t, Double_t> dict_charge = {
        {2212, 1.0},         {-2212, -1.0},       {311, 0},          {321, 1.0},        {-321, -1.0},        {2112, 0.0},
        {-2112, 0.0},        {111, 0.0},          {211, 1.0},        {-211, -1.0},      {1000010020, 1.0},   {1000010030, 1.0},
        {1000020030, 2.0},   {1000020040, 2.0},   {1000020050, 2.0}, {1000030050, 3.0}, {-1000010020, -1.0}, {-1000010030, -1.0},
        {-1000020030, -2.0}, {-1000020040, -2.0}, {1000020050, 2.0}};
    ...
    for (Int_t i_track = 0; i_track < NumTracks; i_track++) {
        ...
        if (TLV_MC_particle.P() > 0.1 && fabs(TLV_MC_particle.Eta()) < 0.85 && MC_N_daughters > 2 && it != dict_mother.end()) {
            ...
            // MC_PDG_code, dict_mother[MC_PDG_code].c_str(), MC_N_daughters, TLV_MC_particle.P());
            ...
            vector<Double_t> fhelix =
                Get_Helix_params_from_kine(TLV_MC_particle, TV3_MC_particle_vertex, (-1.0) * dict_charge[MC_PDG_code]);
            vector<Float_t> params = {(Float_t)fhelix[0], (Float_t)fhelix[1], (Float_t)fhelix[2],
                                      (Float_t)fhelix[3], (Float_t)fhelix[4], (Float_t)fhelix[5]};
    ...
    for (Int_t i_track = 0; i_track < NumTracks; i_track++) {
        ...
        if (TLV_MC_particle.P() > 0.1 && fabs(TLV_MC_particle.Eta()) < 0.85 &&
            std::find(mother_part.begin(), mother_part.end(), MC_index_mother) != mother_part.end()) {

            // printf("\n");
            // printf("Daughter of MC_index: %d, PDG-Code: %d, Index of Daughter: %d, Particle: %s, Momentum of Daughter: %4.3f, Vertex
            // (X,Y,Z):(%4.3f, %4.3f, %4.3f), Radius: %4.3f\n", MC_index_mother, MC_PDG_code, MC_index_particle,
            // dict_low[MC_PDG_code].c_str(), TLV_MC_particle.P(), TV3_MC_particle_vertex.X(), TV3_MC_particle_vertex.Y(),
            // TV3_MC_particle_vertex.Z(), radius);

            // radius cuts to specify candidates that are in the respective TRD layers

            if (radius > 270 && radius < 310) {
                // printf("\n");
                // printf("Daughter of MC_index: %d, Mother-particle: %s, PDG-Code: %d, Index of Daughter: %d, Particle: %s, Momentum of
                // Daughter: %4.3f, Vertex (X,Y,Z):(%4.3f, %4.3f, %4.3f), Radius: %4.3f\n", MC_index_mother,
                // dict_mother[index_to_PDG[MC_index_mother]].c_str(), MC_PDG_code, MC_index_particle, dict_low[MC_PDG_code].c_str(),
                // TLV_MC_particle.P(), TV3_MC_particle_vertex.X(), TV3_MC_particle_vertex.Y(), TV3_MC_particle_vertex.Z(), radius);
    ...
    if ((Int_t)mothers_decay_behind_TRD.size() > 0) {
        ...
        mothers_decay_behind_TRD.erase(std::unique(mothers_decay_behind_TRD.begin(), mothers_decay_behind_TRD.end()),
                                       mothers_decay_behind_TRD.end());
    }
    ...
    if ((Int_t)pos_vertices_behindTRD.size() > 0) {
        ...
        pos_vertices_behindTRD.erase(std::unique(pos_vertices_behindTRD.begin(), pos_vertices_behindTRD.end()),
                                     pos_vertices_behindTRD.end());
    }
...
void Ali_TRD_ST_Analyze::AddVertextoHisto(Long64_t i_event) {
    ...
    std::map<int, std::string> dict_mother = {{
                                                  1000010020,
                                                  "Deuteron",
                                              },
                                              {
    ...
    std::map<int, std::string> dict_low = {{2212, "proton"},  {-2212, "anti-proton"},  {311, "K0"},  {321, "K+"},  {-321, "K-"},
                                           {2112, "neutron"}, {-2112, "anti-neutron"}, {111, "pi0"}, {211, "pi+"}, {-211, "pi-"}};
    ...
    for (Int_t i_track = 0; i_track < NumTracks; i_track++) {
        ...
        Int_t MC_index_particle = TRD_MC_Track->get_index_particle();
        Int_t MC_N_daughters = TRD_MC_Track->get_N_daughters();
        // Double_t       radius                 = TRD_MC_Track -> TMath::Sqrt(TMath::Power(TV3_MC_particle_vertex[0], 2) +
        // TMath::Power(TV3_MC_particle_vertex[1], 2));
    ...
    for (Int_t i_track = 0; i_track < NumTracks; i_track++) {
        ...
        Int_t MC_N_daughters = TRD_MC_Track->get_N_daughters();
        // Double_t       radius                 = TRD_MC_Track -> (TMath::Sqrt(TMath::Power(TV3_MC_particle_vertex[0], 2) +
        // TMath::Power(TV3_MC_particle_vertex[1], 2))); std::vector<float> x_pos_vertices; std::vector<float> y_pos_vertices;
...
std::vector<Int_t> Ali_TRD_ST_Analyze::CompareMCHelizestoTPCHelizes(
    Long64_t i_event, std::vector<pair<std::vector<Float_t>, Int_t>> helix_params_with_index) {
    UShort_t NumTracks = TRD_ST_Event->getNumTracks();  // number of tracks in this event
    Int_t NumTracklets = TRD_ST_Event->getNumTracklets();
    ...
    for (Int_t j = 0; j < (Int_t)helix_params_with_index.size(); j++) {
        ...
        printf("Helix_params of MC-Track: %f, %f, %f, %f, %f, %f\n", helix_param_of_mother[0], helix_param_of_mother[1],
               helix_param_of_mother[2], helix_param_of_mother[3], helix_param_of_mother[4], helix_param_of_mother[5]);
        ...
        for (Int_t i_track = 0; i_track < NumTracks; i_track++) {
            ...
            printf("i_track: %i, Helix_params of TPC-Track: %f, %f, %f, %f, %f, %f\n", i_track, Helix_params[0], Helix_params[1],
                   Helix_params[2], Helix_params[3], Helix_params[4], Helix_params[5]);
            // cut on tal
...
void Ali_TRD_ST_Analyze::DrawTrackletsAroundTPCTracksofCandidates(Long64_t i_event, Int_t apply_Helix_cut,
                                                                  std::vector<Int_t> Found_TPC_Tracks, Int_t graphics, Int_t draw_tracklets,
                                                                  Int_t write_out_bckg, Int_t write_out_sign, Int_t Color = kAzure - 2) {
    ...
    if ((Int_t)Found_TPC_Tracks.size() > 0) {
        for (Int_t i_track = 0; i_track < Num_tracks; i_track++) {
            ...
            if (write_out_bckg || write_out_sign) {
                ...
                TRD_TPC_Track_GNN->setHelix(Helix_params[0], Helix_params[1], Helix_params[2], Helix_params[3], Helix_params[4],
                                            Helix_params[5]);
            ...
            if (Track_sector == 17) {
                search_in_sectors[0] = 0;
                search_in_sectors[1] = 16;
                search_in_sectors[2] = 17;
            } else if (Track_sector == 0) {
                search_in_sectors[0] = 0;
                search_in_sectors[1] = 1;
                search_in_sectors[2] = 17;
            } else {
                search_in_sectors[0] = Track_sector - 1;
                search_in_sectors[1] = Track_sector;
                search_in_sectors[2] = Track_sector + 1;
            }
            ...
            for (Int_t i_tracklet = 0; i_tracklet < NumTracklets; i_tracklet++) {
                ...
                if (graphics && draw_tracklets) {
                    ...
                    vec_TEveLine_tracklets[i_layer][N_tracklets_layers[i_layer]]->SetNextPoint(
                        TV3_offset[0] + scale_length_vec * TV3_dir[0], TV3_offset[1] + scale_length_vec * TV3_dir[1],
                        TV3_offset[2] + scale_length_vec * TV3_dir[2]);

                    // printf("i_tracklet: %d, radius: %4.3f, pos A: {%4.2f, %4.2f, %4.2f}, pos B: {%4.2f, %4.2f, %4.2f}
                    // \n",i_tracklet,radius,TV3_offset[0],TV3_offset[1],TV3_offset[2],TV3_offset[0] +
                    // scale_length_vec*TV3_dir[0],TV3_offset[1] + scale_length_vec*TV3_dir[1],TV3_offset[2] + scale_length_vec*TV3_dir[2]);
...
void Ali_TRD_ST_Analyze::DrawTrackletsAroundVerticesOfNuclearInt(Long64_t i_event, Int_t graphics, Int_t draw_tracklets, Int_t draw_vertex,
    ...
    if ((Int_t)mothers_with_cut.size() > 0) {
        ...
        for (Int_t i = 0; i < (Int_t)mothers_with_cut.size(); i++) {
            ...
            if (draw_MC_tracks && (dict_charge[MC_PDG_Code_mother] > 0 || dict_charge[MC_PDG_Code_mother] < 0)) {
                vector<Double_t> fhelix = Get_Helix_params_from_kine(TLV_MC_particle_mother, TV3_MC_particle_vertex_mother,
                                                                     (-1.0) * dict_charge[MC_PDG_Code_mother]);
                vector<Float_t> params = {(Float_t)fhelix[0], (Float_t)fhelix[1], (Float_t)fhelix[2],
                                          (Float_t)fhelix[3], (Float_t)fhelix[4], (Float_t)fhelix[5]};
                helix_params.push_back(params);
                TRD_ST_TPC_Track_MC->setHelix(fhelix[0], fhelix[1], fhelix[2], fhelix[3], fhelix[4], fhelix[5]);
    ...
    for (Int_t i_pos = 0; i_pos < (Int_t)pos_vertices.size(); i_pos++) {
        ...
        Double_t Radial_pos_vertex =
            TMath::Sqrt(TMath::Power(pos_vertices[i_pos].first[0], 2) + TMath::Power(pos_vertices[i_pos].first[1], 2));
        if (Radial_pos_vertex < 296)
            layer_of_Vertex_in_TRD = 0;
        else if (Radial_pos_vertex > 297 && Radial_pos_vertex < 306)
            layer_of_Vertex_in_TRD = 1;
        else if (Radial_pos_vertex > 310 && Radial_pos_vertex < 320)
            layer_of_Vertex_in_TRD = 2;
            // printf("(X,Y,Z): %f, %f, %f, mother_PDG_code: %d\n", pos_vertices[i_pos].first[0], pos_vertices[i_pos].first[1],
            // pos_vertices[i_pos].first[2], pos_vertices[i_pos].second);
        ...
        if (graphics && draw_vertex) {
            TEveP_nuclear_vertex_positions->SetNextPoint(pos_vertices[i_pos].first[0], pos_vertices[i_pos].first[1],
                                                         pos_vertices[i_pos].first[2]);
        ...
        for (Int_t i_tracklet = 0; i_tracklet < NumTracklets; i_tracklet++) {
            ...
            Double_t radius_local = TMath::Sqrt( TMath::Power(difference_vector.X(), 2) + TMath::Power(difference_vector.Y(),2) +
            TMath::Power(difference_vector.Z(), 2));
            ...
            for (Int_t j=0; j < (Int_t) helix_params.size(); j++){
                Float_t new_distance = distance_circ_point_2D_new(TV3_offset[0], TV3_offset[1], (Double_t) helix_params[j][5], (Double_t)
            helix_params[j][0], (Double_t) 1/helix_params[j][4]); if (new_distance < closest_distance){ closest_distance = new_distance;
            ...
            */
            ...
            for (Int_t j = 0; j < (Int_t)helix_params.size(); j++) {
                ...
                Float_t d_xy;
                Float_t d_z;
                new_helix->setHelix(helix_params[j][0], helix_params[j][1], helix_params[j][2], helix_params[j][3], helix_params[j][4],
                                    helix_params[j][5]);
            ...
            Float_t comp_dist = TMath::Sqrt(TMath::Power(distance_xy, 2) + TMath::Power(distance_z, 2));
            // printf("Number_tracklet: %i, distance xy: %f, distance z: %f, cylinder radius: %f\n", i_tracklet, distance_xy, distance_z,
            // comp_dist);
            if (apply_Helix_cut) {
                if (comp_dist > 30) {
                    continue;
            ...
            if (graphics && draw_tracklets) {
                ...
                vec_TEveLine_tracklets[i_layer][N_tracklets_layers[i_layer]]->SetNextPoint(TV3_offset[0], TV3_offset[1], TV3_offset[2]);
                vec_TEveLine_tracklets[i_layer][N_tracklets_layers[i_layer]]->SetNextPoint(TV3_offset[0] + scale_length_vec * TV3_dir[0],
                                                                                           TV3_offset[1] + scale_length_vec * TV3_dir[1],
                                                                                           TV3_offset[2] + scale_length_vec * TV3_dir[2]);

                // printf("i_tracklet: %d, radius: %4.3f, pos A: {%4.2f, %4.2f, %4.2f}, pos B: {%4.2f, %4.2f, %4.2f}
                // \n",i_tracklet,radius,TV3_offset[0],TV3_offset[1],TV3_offset[2],TV3_offset[0] + scale_length_vec*TV3_dir[0],TV3_offset[1]
                // + scale_length_vec*TV3_dir[1],TV3_offset[2] + scale_length_vec*TV3_dir[2]);
...
Int_t Ali_TRD_ST_Analyze::Draw_event(Long64_t i_event, Int_t graphics, Int_t draw_tracks, Int_t draw_tracklets, Double_t track_path,
    ...
    for (Int_t i_tracklet = 0; i_tracklet < NumTracklets; i_tracklet++) {
        ...
        printf("i_tracklet: %d, pos: {%4.3f, %4.3f, %4.3f}, trk_index: %d, q: {%4.2f, %4.2f, %4.2f} \n", i_tracklet, TV3_offset.X(),
               TV3_offset.Y(), TV3_offset.Z(), trk_index, ADC_val_q0, ADC_val_q1, ADC_val_q2);
...
Int_t Ali_TRD_ST_Analyze::Do_TPC_TRD_matching(Long64_t i_event, Double_t xy_matching_window, Double_t z_matching_window, Int_t graphics) {
    ...
    for (Int_t i_tracklet = 0; i_tracklet < NumTracklets; i_tracklet++) {
        ...
        // for (Int_t i_time = 0; i_time < 24; i_time++) { vec_ADC_val[i_det][i_tracklet][i_time]        = TRD_ST_Tracklet
        // ->get_ADC_val(i_time); }
    ...
    // for(Int_t i_track = 3; i_track < 4; i_track++)
    {
        ...
        for (Int_t i_layer = 0; i_layer < 6; i_layer++) {
            ...
            TPC_single_helix->setHelix(TRD_ST_TPC_Track->getHelix_param(0), TRD_ST_TPC_Track->getHelix_param(1),
                                       ...
                                       TRD_ST_TPC_Track->getHelix_param(4), TRD_ST_TPC_Track->getHelix_param(5));
            Float_t pathA_dca, dcaAB_dca;
            fHelixAtoPointdca(vec_TV3_offset_tracklets[det_best][tracklet_best], TPC_single_helix, pathA_dca,
                              dcaAB_dca);  // new helix to point dca calculation
            Double_t track_pos_at_tracklet[3];
            TRD_ST_TPC_Track->Evaluate(pathA_dca, track_pos_at_tracklet);
            // printf("%s i_layer: %d %s, i_track: %d, dcaAB_dca: %4.3f, offset_point: {%4.3f, %4.3f, %4.3f}, TPC point: {%4.3f, %4.3f,
            // %4.3f}
...
#if defined(USEEVE)
            if (graphics)
                TEveP_TPC_at_offset_points->SetPoint(TPC_at_offset_point, track_pos_at_tracklet[0], track_pos_at_tracklet[1],
                                                     track_pos_at_tracklet[2]);
#endif
            TPC_at_offset_point++;
            ...
            if (graphics) {
                ...
                vec_TEveLine_tracklets_match[i_layer][size_tracklet]->SetNextPoint(vec_TV3_offset_tracklets[det_best][tracklet_best][0],
                                                                                   vec_TV3_offset_tracklets[det_best][tracklet_best][1],
                                                                                   vec_TV3_offset_tracklets[det_best][tracklet_best][2]);
                vec_TEveLine_tracklets_match[i_layer][size_tracklet]->SetNextPoint(
                    vec_TV3_offset_tracklets[det_best][tracklet_best][0] +
                        scale_length_vec * vec_TV3_dir_tracklets[det_best][tracklet_best][0],
                    vec_TV3_offset_tracklets[det_best][tracklet_best][1] +
                        scale_length_vec * vec_TV3_dir_tracklets[det_best][tracklet_best][1],
                    vec_TV3_offset_tracklets[det_best][tracklet_best][2] +
                        scale_length_vec * vec_TV3_dir_tracklets[det_best][tracklet_best][2]);
                ...
                TEveP_offset_points->SetPoint(offset_point, vec_TV3_offset_tracklets[det_best][tracklet_best][0],
                                              vec_TV3_offset_tracklets[det_best][tracklet_best][1],
                                              vec_TV3_offset_tracklets[det_best][tracklet_best][2]);
        ...
        // if(N_matched_tracklets >= 4 && vec_idx_matched_TPC_track[tracks_size] == 25)
        // Draw_TPC_track(vec_idx_matched_TPC_track[tracks_size],kAzure-2,2,1000.0); printf(" \n");
        //--------------------------------------------------
            ...
            || (abs(nsigma_TPC_pi) > 2.5 && abs(nsigma_TPC_p) > 2.5 && abs(nsigma_TPC_K) < 2.5))  // K
        {
            for (Int_t i_layer = 0; i_layer < (Int_t)matched_tracks[i_TPC_track].size(); i_layer++) {
                ...
                if (matched_tracks[i_TPC_track][i_layer] != NULL) {
                    ...
                    Ali_TRD_ST_Tracklets* TRD_ST_Tracklet_adc =
                        TRD_ST_Event->getTracklet((Int_t)matched_tracks[i_TPC_track][i_layer]->get_TRD_index());
                    ...
                    if (tb_count == 23) {
                        ...
                        Double_t scale_factor =
                            3.7 * ((*vec_TV3_TRD_center[det][2]).Mag()) / ((*vec_TV3_TRD_center[det][2]).Dot(TV3_dir_matched));
                        ...
                        Double_t tracklet_length = TMath::Sqrt(TMath::Power(scale_factor * TV3_dir_matched[0], 2) +
                                                               TMath::Power(scale_factor * TV3_dir_matched[1], 2) +
                                                               TMath::Power(scale_factor * TV3_dir_matched[2], 2));
                        // same as scale factor

                        // vec_TEveLine_tracklets[i_layer][N_tracklets_layers[i_layer]]
                        // ->SetNextPoint(TV3_offset[0],TV3_offset[1],TV3_offset[2]);
                        // vec_TEveLine_tracklets[i_layer][N_tracklets_layers[i_layer]] ->SetNextPoint(TV3_offset[0] +
                        // scale_length_vec*TV3_dir[0],TV3_offset[1] + scale_length_vec*TV3_dir[1],TV3_offset[2] +
                        // scale_length_vec*TV3_dir[2]); if (tracklet_length > 10.0) continue;
                        // printf("\ntracklet TV3_dir_matched[0]: %4.3f, TV3_dir_matched[1]: %4.3f, TV3_dir_matched[2]: %4.3f, dot: %4.3f,
                        // tracklet_length: %4.3f
                        ...
                        // TMath::Power(3.0*TV3_dir_matched[0],2) + TMath::Power(3.0*TV3_dir_matched[1],2) +
                        // TMath::Power(3.0*TV3_dir_matched[2],2),TMath::Sqrt( TMath::Power(3.0*TV3_dir_matched[0],2) +
                        // TMath::Power(3.0*TV3_dir_matched[1],2) + TMath::Power(3.0*TV3_dir_matched[2],2)));

                        //---------------calculate local position here ----------------------

                        // vec_TV3_TRD_center
                        TVector3 local_little_vec =
                            (*vec_TV3_TRD_offset[det]) - TV3_offset_matched;  // pointing from center of the chamber to the datapoint?
    ...
    for (Int_t i_Track = 0; i_Track < (Int_t)found_tracks.size(); i_Track++) {
        ...
        for (Int_t i_lay = 0; i_lay < 6; i_lay++) {
            if (found_tracks[i_Track][i_lay] != NULL) {
                ...
                TEveLine_Kalman_found[i_Track][i_lay]->SetNextPoint(found_tracks[i_Track][i_lay]->get_TV3_offset()[0],
                                                                    found_tracks[i_Track][i_lay]->get_TV3_offset()[1],
                                                                    found_tracks[i_Track][i_lay]->get_TV3_offset()[2]);
                TEveLine_Kalman_found[i_Track][i_lay]->SetNextPoint(
                    found_tracks[i_Track][i_lay]->get_TV3_offset()[0] + scale_length_vec * found_tracks[i_Track][i_lay]->get_TV3_dir()[0],
                    found_tracks[i_Track][i_lay]->get_TV3_offset()[1] + scale_length_vec * found_tracks[i_Track][i_lay]->get_TV3_dir()[1],
                    found_tracks[i_Track][i_lay]->get_TV3_offset()[2] + scale_length_vec * found_tracks[i_Track][i_lay]->get_TV3_dir()[2]);
#endif
...
#if defined(USEEVE)
                TEveLine_Kalman_found[i_Track][i_lay] = new TEveLine();
                TEveLine_Kalman_found[i_Track][i_lay]->SetNextPoint(vec_kalman_TRD_trackets[i_Track][i_lay]->get_TV3_offset()[0],
                                                                    vec_kalman_TRD_trackets[i_Track][i_lay]->get_TV3_offset()[1],
                                                                    vec_kalman_TRD_trackets[i_Track][i_lay]->get_TV3_offset()[2]);
                TEveLine_Kalman_found[i_Track][i_lay]->SetNextPoint(
                    vec_kalman_TRD_trackets[i_Track][i_lay]->get_TV3_offset()[0] +
                        scale_length_vec * vec_kalman_TRD_trackets[i_Track][i_lay]->get_TV3_dir()[0],
                    vec_kalman_TRD_trackets[i_Track][i_lay]->get_TV3_offset()[1] +
                        scale_length_vec * vec_kalman_TRD_trackets[i_Track][i_lay]->get_TV3_dir()[1],
                    vec_kalman_TRD_trackets[i_Track][i_lay]->get_TV3_offset()[2] +
                        scale_length_vec * vec_kalman_TRD_trackets[i_Track][i_lay]->get_TV3_dir()[2]);
#endif
...
TH1I* Ali_TRD_ST_Analyze::get_h_good_bad_TRD_chambers() { return h_good_bad_TRD_chambers; }
...
void Ali_TRD_ST_Analyze::Calibrate(Int_t graphics) {
    ...
    if ((Int_t)vec_tp_Delta_vs_impact.size() == 0 && (Int_t)vec_tp_Delta_vs_impact_circle.size() == 0) {
        ...
        for (Int_t i_det = 0; i_det < 540; i_det++) {
            vec_tp_Delta_vs_impact[i_det] =
                new TProfile(Form("vec_th1d_Delta_vs_impact_%d", i_det), Form("vec_th1d_Delta_vs_impact_%d", i_det), 360, -360, 360);
            vec_TH2D_Delta_vs_impact[i_det] =
                new TH2D(Form("vec_th2d_Delta_vs_impact_%d", i_det), Form("vec_th2d_Delta_vs_impact_%d", i_det), 10, 70, 110, 50, -25, 25);
            vec_tp_Delta_vs_impact_circle[i_det] = new TProfile(Form("vec_th1d_Delta_vs_impact_circle_%d", i_det),
                                                                Form("vec_th1d_Delta_vs_impact_circle_%d", i_det), 360, -360, 360);
            vec_TH2D_Delta_vs_impact_circle[i_det] =
                new TH2D(Form("vec_th2d_Delta_vs_impact_circle_%d", i_det), Form("vec_th2d_Delta_vs_impact_circle_%d", i_det), 13, 70, 110,
                         200, -100, 100);
        }
    }
    TVector3 vec_TV3_tracklet_vectors, TV3_tracklet_off_vector, vec_dir_vec_circle;
    ...
    // for(Int_t i_track = 0; i_track < 1; i_track++)
    {
        ...
        for (Int_t i_layer = 0; i_layer < 6; i_layer++) {
            ...
            // printf("Kalman: track_posAX = %4.3f, track_posBX = %4.3f, track_posAY = %4.3f, track_posBY =
            ...
            Double_t impact_angle_circle =
                vec_dir_vec_circle.Angle((*vec_TV3_TRD_center[detector[i_layer]][2]));  // i need something like that for circles
            ...
            // if(impact_angle_circle*TMath::RadToDeg() > 85.0 && impact_angle_circle*TMath::RadToDeg() < 90.0)
            // printf("delta_x_local_tracklet = %4.3f, sign_angle_circle: %4.3f, Delta_angle_circle: %4.3f
            ...
            vec_TH2D_Delta_vs_impact[detector[i_layer]]->Fill(impact_angle_circle * TMath::RadToDeg(),
                                                              Delta_angle_circle * TMath::RadToDeg());
            vec_tp_Delta_vs_impact[detector[i_layer]]->Fill(impact_angle_circle * TMath::RadToDeg(),
                                                            Delta_angle_circle * TMath::RadToDeg());
            // if(impact_angle_circle*TMath::RadToDeg() > 85.0 && impact_angle_circle*TMath::RadToDeg() < 90.0) printf("--> i_track: %d,
            ...
            if (graphics) {
                ...
                TEveLine_vec_dir_vec_circle[i_track][i_layer]->SetNextPoint(vec_kalman_TRD_trackets[i_track][i_layer]->get_TV3_offset()[0],
                                                                            vec_kalman_TRD_trackets[i_track][i_layer]->get_TV3_offset()[1],
                                                                            vec_kalman_TRD_trackets[i_track][i_layer]->get_TV3_offset()[2]);
                TEveLine_vec_dir_vec_circle[i_track][i_layer]->SetNextPoint(
                    vec_kalman_TRD_trackets[i_track][i_layer]->get_TV3_offset()[0] + scale_length_vec * vec_dir_vec_circle.X(),
                    vec_kalman_TRD_trackets[i_track][i_layer]->get_TV3_offset()[1] + scale_length_vec * vec_dir_vec_circle.Y(),
                    vec_kalman_TRD_trackets[i_track][i_layer]->get_TV3_offset()[2]);
            }
#endif
        ...
        Double_t D_help = (x1 * x1 + y1 * y1) * (x3 * y2 - x2 * y3) + (x2 * x2 + y2 * y2) * (x1 * y3 - x3 * y1) +
        ...
        for (Int_t i_layer = 0; i_layer < 6; ++i_layer) {
            vec_dir_vec_circle_circle[i_layer].SetXYZ(-999.0, -999.0,
                                                      -999.0);  // because i maybe need it to have all 6 dimensions in Calibrate
        }
        ...
        if (graphics) {
            ...
            for (Double_t i_sign = -1.0; i_sign <= +1.0; i_sign += 2.0) {
                for (Double_t i_y = ((par_circle[1] - par_circle[2]) + delta_i_y); i_y < ((par_circle[1] + par_circle[2]) - delta_i_y);
                     (i_y += delta_i_y)) {
                    Double_t i_x =
                        i_sign * TMath::Sqrt(TMath::Power(par_circle[2], 2.0) - TMath::Power(i_y - par_circle[1], 2.0)) + par_circle[0];
        ...
        // for(Int_t i_point = 0; i_point < 6; i_point++)
        {
            ...
            // printf("vec_dir_vec_circle_notempty[i_point].X: %4.3f, vec_dir_vec_circle_notempty[i_point].Y: %4.3f,
            // vec_dir_vec_circle_notempty[i_point].Z: %4.3f
            ...
            // x_valB*x_valB,y_val*y_val + x_val*x_val);
            ...
            Double_t delta_x_local_global_circle_circle =
                vec_dir_vec_circle_circle[i_layer].Dot((*vec_TV3_TRD_center[detector[i_layer]][0]));
            ...
            Double_t impact_angle_circle_circle = vec_dir_vec_circle_circle[i_layer].Angle(
                (*vec_TV3_TRD_center[detector[i_layer]][2]));  // i need something like that for circles
            ...
            Double_t Delta_angle_circle_circle =
                sign_angle_circle_circle * vec_dir_vec_circle_circle[i_layer].Angle(vec_TV3_tracklet_vectors);

            // printf("   ---> vec_KF: {%4.3f, %4.3f, %4.3f}, vec_trkl: {%4.3f, %4.3f, %4.3f}
            // \n",vec_dir_vec_circle.X(),vec_dir_vec_circle.Y(),vec_dir_vec_circle.Z(),vec_TV3_tracklet_vectors.X(),vec_TV3_tracklet_vectors.Y(),vec_TV3_tracklet_vectors.Z());
            // if(impact_angle_circle_circle*TMath::RadToDeg() > 85.0 && impact_angle_circle_circle*TMath::RadToDeg() < 90.0)
            // printf("delta_x_local_tracklet_circle_circle = %4.3f, sign_angle_circle_circle: %4.3f, Delta_angle_circle_circle: %4.3f
            ...
            // if(impact_angle_circle_circle*TMath::RadToDeg() > 85.0 && impact_angle_circle_circle*TMath::RadToDeg() < 90.0)
            // printf("i_layer: %d, impact angle circle: %4.3f, delta angle circle: %4.3f
            // \n",i_layer,impact_angle_circle_circle*TMath::RadToDeg(),Delta_angle_circle_circle*TMath::RadToDeg());

            vec_TH2D_Delta_vs_impact_circle[detector[i_layer]]->Fill(impact_angle_circle_circle * TMath::RadToDeg(),
                                                                     Delta_angle_circle_circle * TMath::RadToDeg());
            vec_tp_Delta_vs_impact_circle[detector[i_layer]]->Fill(impact_angle_circle_circle * TMath::RadToDeg(),
                                                                   Delta_angle_circle_circle * TMath::RadToDeg());
            ...
            if (graphics) {
                ...
                TEveLine_vec_dir_vec_circle_circle[i_track][i_layer]->SetNextPoint(
                    vec_kalman_TRD_trackets[i_track][i_layer]->get_TV3_offset()[0],
                    vec_kalman_TRD_trackets[i_track][i_layer]->get_TV3_offset()[1],
                    vec_kalman_TRD_trackets[i_track][i_layer]->get_TV3_offset()[2]);
                TEveLine_vec_dir_vec_circle_circle[i_track][i_layer]->SetNextPoint(
                    vec_kalman_TRD_trackets[i_track][i_layer]->get_TV3_offset()[0] +
                        scale_length_vec * vec_dir_vec_circle_circle[i_layer].X(),
                    vec_kalman_TRD_trackets[i_track][i_layer]->get_TV3_offset()[1] +
                        scale_length_vec * vec_dir_vec_circle_circle[i_layer].Y(),
                    vec_kalman_TRD_trackets[i_track][i_layer]->get_TV3_offset()[2]);
            }
#endif
        ...
        for (Int_t i_sec_sub = 0; i_sec_sub < 3; i_sec_sub++) {
            ...
            for (Int_t i_stack = 0; i_stack < 5; i_stack++) {
                ...
                for (Int_t i_layer = 0; i_layer < 6; i_layer++) {
                    ...
                    HistName += NoP;
                    // plotTopLegend((char*)HistName.Data(),0.24,0.89-i_layer*0.07,0.045,arr_color_layer[i_layer],0.0,42,1,1); // char*
                    // label,Float_t x=-1,Float_t y=-1, Float_t size=0.06,Int_t color=1,Float_t angle=0.0, Int_t font = 42, Int_t NDC = 1,
                    // Int_t align = 1
                }
            }
        }
