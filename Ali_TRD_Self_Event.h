...
class Ali_Kalman_Track : public TObject {
   ...
   public:
    Ali_Kalman_Track() : TLV_part(), TRD_ADC_time_layer(), aliHelix_params(), Chi_2(-1), Id(0) {}
    ~Ali_Kalman_Track() {}
   ...
   public:
    Ali_TRD_Photon()
        : vertex_point(),
          ...
          dcaAB_z(-1),
          dcaAB(-1),
          Inv_mass_AB_Lambda(-1),
          Inv_mass_AB_antiLambda(-1),
          fNumTPC_Tracks(0),
          fNumKalman_Tracks(0),
          TLV_part_A(),
          TLV_part_B() {
    ...
    // setters
    void set_vertex_point(Float_t x, Float_t y, Float_t z) {
        vertex_point[0] = x;
        vertex_point[1] = y;
        vertex_point[2] = z;
    }
    ...
    Ali_TPC_Track* createTPC_Track() {
        if (fNumTPC_Tracks == fTPC_Tracks->GetSize()) fTPC_Tracks->Expand(fNumTPC_Tracks + 10);
    ...
    UShort_t getNumTPC_Tracks() const { return fNumTPC_Tracks; }
    Ali_TPC_Track* getTPC_Track(UShort_t i) const { return i < fNumTPC_Tracks ? (Ali_TPC_Track*)((*fTPC_Tracks)[i]) : NULL; }
    ...
    Ali_Kalman_Track* createKalman_Track() {
        if (fNumKalman_Tracks == fKalman_Tracks->GetSize()) fKalman_Tracks->Expand(fNumKalman_Tracks + 10);
    ...
    UShort_t getNumKalman_Tracks() const { return fNumKalman_Tracks; }
    Ali_Kalman_Track* getKalman_Track(UShort_t i) const { return i < fNumKalman_Tracks ? (Ali_Kalman_Track*)((*fKalman_Tracks)[i]) : NULL; }
...
class Ali_TRD_Nuclear_interaction : public TObject {
   ...
   public:
    ...
    // setters
    void set_avg_sec_vertex(Float_t x, Float_t y, Float_t z) {
        avg_sec_vertex[0] = x;
        avg_sec_vertex[1] = y;
        avg_sec_vertex[2] = z;
    }
...
class Ali_TRD_Self_Event : public TObject {
   ...
   public:
    ...
    {
        if (fNumTRD_Photons == fPhotons->GetSize()) fPhotons->Expand(fNumTRD_Photons + 10);
    ...
    Int_t getNumPhotons() const { return fNumTRD_Photons; }
    Ali_TRD_Photon* getPhoton(Int_t i) const { return i < fNumTRD_Photons ? (Ali_TRD_Photon*)((*fPhotons)[i]) : NULL; }
    //----------------------------
    ...
    UShort_t getNumNucInteractions() const { return fNumTRD_Nuclear_interactions; }
...
class Ali_Helix_copy : public TObject {
   ...
   public:
    Ali_Helix_copy() : aliHelix_params() {}
    ~Ali_Helix_copy() {}
