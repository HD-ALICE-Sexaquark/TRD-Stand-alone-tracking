...
void Macro_TRD_tracking(TString input_list = "run0_test.txt", Int_t event_plot = -1, Double_t track_path = 1000.0,
                        Double_t beam_path = 0.0) {
    ...
    Int_t bool_make_invariant_mass_hist_up_to_layer =
        2;  // calculate invariant mass of S particle up to generation <Int_t> and draws a histogram
    ...
    for (Long64_t event = start_event; event < stop_event; event++) {
        ...
        if (event != 0 && event % 50 == 0) cout << "." << flush;
        ...
        if (graphics)
            TRD_ST_Analyze->Draw_event(event, graphics, draw_all_TPC_tracks, draw_all_tracklets, track_path,
                                       draw_TRD_digits);  // ->draws TPC tracks
        ...
        TRD_ST_Analyze->Do_TPC_TRD_matching(event, 3.0, 10.0,
                                            graphics * draw_tracklets_TPC_match);  // last one is graphics  --> draws kalman TRD tracklets
        ...
        if (KF_tracker) {
            tracker_found_tracklets_KF = kalid.Kalman_Trackfind(TRD_ST_Analyze->Tracklets, TRD_ST_Analyze->Number_Tracklets,
                                                                use_prim_vertex);  // 0 = no primary vertex, 1 = primary vertex used
            ...
            if (graphics && draw_found_tracklets)
                TRD_ST_Analyze->Draw_Kalman_Tracklets(tracker_found_tracklets_KF);  // Draws the Kalman matched TRD tracklets
            if (graphics && draw_all_TRD_tracks)
                TRD_ST_Analyze->Draw_Kalman_Helix_Tracks(-1, kGreen, 5.0, 500.0);  // -1 -> all kalman tracks drawn, 3.0, 500.0
        ...
        Int_t found_good_AP_vertex_TPC = TRD_ST_Analyze->Calculate_secondary_vertices(
            graphics * draw_secondary_vertices, 0, 0);  // (0 = no graphics), (0 = TRD, 1 = TPC), flag_fill_tree (0 don't fill)
        Int_t found_good_AP_vertex_TRD = TRD_ST_Analyze->Calculate_secondary_vertices(
            graphics * draw_secondary_vertices, 1, 1);  // (0 = no graphics), (0 = TRD, 1 = TPC), flag_fill_tree (0 don't fill)
